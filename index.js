...
app.get('/api/club/ranks', strictLimiter, authenticateFinancialRequest, async (req, res) => {
  try {
    const { leagueOrder, limit: limitParam } = req.query;
    const db = admin.firestore();
    
    let clubsQuery = db.collection('clubs');
    if (leagueOrder) {
      clubsQuery = clubsQuery.where('currentLeagueOrder', '==', parseInt(leagueOrder, 10));
    }
    ...
    snapshot.forEach(doc => {
      const data = doc.data();
      // Only include public info, intentionally leaving out inviteCode
      clubs.push({
        id: doc.id,
        name: data.name,
        ...
        level: data.level || Math.floor((data.totalPoints || 0) / 100) + 1
      });
    });
    ...
    const parsedLimit = parseInt(limitParam, 10) || 100;
    const limitedClubs = clubs.slice(0, parsedLimit);

    res.status(200).json({ success: true, clubs: limitedClubs });
  } catch (error) {
    console.error('Error fetching club ranks:', error.message);
    res.status(400).json({ success: false, error: error.message });
  }
});
