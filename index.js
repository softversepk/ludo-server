...
// Secure Friend Chat Endpoint
app.post('/api/chat/send', strictLimiter, authenticateFinancialRequest, async (req, res) => {
  try {
    ...
    // Validate users exist and are friends (Optional, but adds security)
    const senderDoc = await db.collection('users').doc(senderId).get();
    if (!senderDoc.exists) return res.status(404).json({ success: false, error: 'Sender not found' });
    ...
    // Sort IDs to create a consistent chat room ID
    ...
    batch.set(chatRoomRef, {
      ...
      createdAt: admin.firestore.FieldValue.serverTimestamp(), // Will only set if doesn't exist
    }, { merge: true });
...
app.post('/api/game-invite/send', strictLimiter, authenticateFinancialRequest, async (req, res) => {
  try {
    const { fromUserId, toUserId, gameData } = req.body;
    
    if (fromUserId !== req.userId) {
      return res.status(403).json({ success: false, error: 'Unauthorized user' });
    }

    if (!toUserId || !gameData) {
      return res.status(400).json({ success: false, error: 'Invalid invite data' });
    }

    const db = admin.firestore();
    const rtdb = admin.database();

    // Verify sender exists and has enough coins/diamonds for bet
    const userDoc = await db.collection('users').doc(fromUserId).get();
    if (!userDoc.exists) throw new Error('User not found');
    
    const userData = userDoc.data();

    // Create RTDB room for game
    const roomRef = rtdb.ref('gameRooms').push();
    const roomCode = roomRef.key.substring(0, 6).toUpperCase();
    ...
    // Create invite in RTDB for the target user
    ...
    await inviteRef.set({
      ...
      fromUsername: userData.username || 'Player',
      fromAvatar: userData.avatar || 'default',
      toUserId: toUserId,
      gameType: gameData.gameType,
...
app.post('/api/game-invite/accept', strictLimiter, authenticateFinancialRequest, async (req, res) => {
  try {
    ...
    // Keep it for a short time or remove immediately
    await inviteRef.remove();
...
app.post('/api/game-invite/reject', strictLimiter, authenticateFinancialRequest, async (req, res) => {
  try {
    ...
    // Assuming we also want to remove the room
    if (invite.roomCode) {
      await rtdb.ref(`gameRooms/${invite.roomCode}`).remove();
    }
...
app.post('/api/game-invite/cancel', strictLimiter, authenticateFinancialRequest, async (req, res) => {
  try {
    const { inviteId, toUserId, roomCode } = req.body;
    const userId = req.userId; // the sender
