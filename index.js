...
io.on("connection", (socket) => {
  ...
  socket.on("update_game_state", ({ roomCode, gameState }, callback) => {
    ...
    if (room) {
      ...
      // SECURITY: Validate Human Player Dice Roll
      // Check if the update contains a new dice roll for the current player
      if (
        gameState.lastDiceValues && 
        room.gameState && 
        room.gameState.lastDiceValues &&
        room.gameState.currentPlayer
      ) {
        const currentPlayer = room.gameState.currentPlayer;
        const newDice = gameState.lastDiceValues[currentPlayer];
        const oldDice = room.gameState.lastDiceValues[currentPlayer];
        ...
        if (newDice && newDice !== oldDice) {
          if (room.expectedDiceRoll && room.expectedDiceRoll.playerColor === currentPlayer) {
            ...
            if (Date.now() - expected.timestamp < 30000) {
              if (newDice !== expected.diceValue) {
                console.warn(`[SECURITY] Blocked hacked human dice roll in room ${roomCode}. Expected ${expected.diceValue}, got ${newDice}`);
                if (callback) callback({ success: false, error: "Invalid dice roll detected" });
                return;
              }
              // Clear expected dice roll after successful validation to prevent replay
              room.expectedDiceRoll = null;
            } else {
              console.warn(`[SECURITY] Blocked expired human dice roll in room ${roomCode}`);
              if (callback) callback({ success: false, error: "Dice roll expired" });
              return;
            }
          } else {
            // Strict Security: Block any new dice roll that wasn't generated by the server!
            // Exception: If the player is a bot, it's handled below by expectedBotMove
            const isBot = room.players && room.players[currentPlayer] && room.players[currentPlayer].isBot;
            if (!isBot && !String(currentPlayer).startsWith('bot_')) {
              console.warn(`[SECURITY] Blocked unverified human dice roll in room ${roomCode}. No server-generated roll expected.`);
              if (callback) callback({ success: false, error: "Unverified dice roll" });
              return;
            }
          }
        }
  ...
  // SECURE DICE ROLL FOR HUMAN PLAYERS
  socket.on("get_secure_dice_roll", ({ roomCode, playerKey }, callback) => {
    try {
      ...
      if (callback) callback({ success: true, diceValue });
    } catch (err) {
