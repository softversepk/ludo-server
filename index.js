...
// SECURE BACKEND LEAGUE RESET (Bank-Level Security)
app.post('/api/club/process-weekend', strictLimiter, authenticateFinancialRequest, async (req, res) => {
  try {
    ...
    // Secure Check 1: Already processed this cycle?
    const metaDocRef = db.collection('systemSettings').doc('leagueMetadata');
    ...
    if (metaDoc.exists) {
      ...
      if (data.lastProcessedLeagueId === leagueId) {
        return res.status(400).json({ success: false, error: 'Already processed this cycle' });
      }
    }

    // Secure Check 2: Has the league cycle actually ended?
    const configDoc = await db.collection('systemSettings').doc('leagueConfig').get();
    let config = { type: 'day_to_day', startDay: 1, durationDays: 7 };
    if (configDoc.exists) {
    ...
    if (config.type === '1_minute') {
      ...
      nextReset = weekEnd;
    } else if (config.type === 'days' && config.lastResetAt) {
      const start = new Date(config.lastResetAt);
      const cycleDuration = (config.durationDays || 7) * 24 * 60 * 60 * 1000;
      ...
      nextReset = new Date(currentStart.getTime() + cycleDuration);
    } else {
      ...
      nextReset = new Date(weekStart);
      nextReset.setDate(nextReset.getDate() + 7);
    }

    // Allow 2 minutes leeway for network latency and timer mismatches
    const timeRemaining = nextReset.getTime() - now.getTime();
    if (timeRemaining > 120000 && config.type !== '1_minute') {
      console.warn(`⚠️ [SECURITY] Attempted early reset. Time remaining: ${timeRemaining}ms`);
      return res.status(400).json({ success: false, error: 'League cycle has not ended yet' });
    }

    // Lock processing
    await metaDocRef.set({ isProcessing: true }, { merge: true });

    try {
      console.log('🏆 [SERVER SECURE] Processing league weekend...');
      ...
      // Process each league
      for (let leagueOrder = 1; leagueOrder <= 20; leagueOrder++) {
        const clubs = clubsByLeague[leagueOrder] || [];
        if (clubs.length === 0) continue;

        // Sort by weekly points descending
        clubs.sort((a, b) => (b.weeklyPoints || 0) - (a.weeklyPoints || 0));

        let CLUB_LEAGUES = [];
        try {
          CLUB_LEAGUES = require('./utils/clubLeagues');
        } catch (e) {
          console.warn('⚠️ Could not load clubLeagues for rewards');
        }
        const leagueConfig = CLUB_LEAGUES.find(l => l.order === leagueOrder);
        ...
        for (let index = 0; index < clubs.length; index++) {
          ...
          let nextLeagueOrder = leagueOrder;
          let isPromoted = false;
          let isDemoted = false;

          // Promote top 3
          if (rank <= 3 && leagueOrder < 20) {
            nextLeagueOrder = leagueOrder + 1;
            isPromoted = true;
          } 
          // Demote 4th and below
          else if (rank >= 4 && leagueOrder > 1) {
            nextLeagueOrder = leagueOrder - 1;
            isDemoted = true;
          }

          // Distribute rewards
          if (leagueConfig && rank <= 4) {
            const rewardRank = rank;
            const reward = leagueConfig.rewards[rewardRank];
            if (reward) {
              const membersSnap = await db.collection('users').where('clubId', '==', club.id).get();
              membersSnap.docs.forEach(memberDoc => {
                const memberRef = db.collection('users').doc(memberDoc.id);
          ...
          const updateData = {
            currentLeagueOrder: nextLeagueOrder,
            previousLeagueOrder: leagueOrder,
            weeklyPoints: 0,
            lastWeekPoints: currentPoints,
            pointsResetAt: admin.firestore.FieldValue.serverTimestamp()
          };

          if (isPromoted) updateData.lastPromotedAt = admin.firestore.FieldValue.serverTimestamp();
          if (isDemoted) updateData.lastDemotedAt = admin.firestore.FieldValue.serverTimestamp();
      ...
      await metaDocRef.set({
        isProcessing: false,
        lastProcessedLeagueId: leagueId,
        lastProcessedAt: admin.firestore.FieldValue.serverTimestamp()
      }, { merge: true });

      // Trigger socket event to notify clients to refresh
      if (io) {
        io.emit('league_reset', { leagueId, timestamp: Date.now() });
      }

      console.log(`✅ [SERVER SECURE] Successfully reset ${resetCount} clubs`);
      res.json({ success: true, resetCount });

    } catch (err) {
      // Release lock on error
      await metaDocRef.set({ isProcessing: false }, { merge: true });
      throw err;
    }

  } catch (error) {
    console.error('❌ [SERVER SECURE] Error processing club weekend:', error);
